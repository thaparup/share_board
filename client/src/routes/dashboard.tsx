        ...
        <Body>
            <div className="p-6 space-y-6">
                            ...
                            {data?.data.assignedTasks.map((task) => (
                                <div key={task.id} className="flex items-center justify-between p-3 hover:bg-gray-50 rounded-lg">
                                    <div className="flex-1">
                                        ...
                                        <p className="text-xs text-gray-500">{task.workspaceName}</p>
                                    </div>
                ...
                <div className="bg-white rounded-lg shadow-sm border">
                    ...
                    <div className="p-6">
                        <div className="space-y-3">
                            {data?.data.upcomingDeadlines && data?.data.upcomingDeadlines?.map((task) => (
                                <div key={task.id} className="flex items-center justify-between p-3 hover:bg-gray-50 rounded-lg">
                                    <div>
                                        <h4 className="text-sm font-medium text-gray-900">{task.name}</h4>
                                        <span className={`px-2 py-1 text-xs font-medium rounded-full ${getPriorityColor(task.priority)}`}>
                        ...
                        <div className="p-6 ">
                            <div className=" w-full">
                                {data?.data?.workspaces.map((workspace) => {
                                    ...
                                    return (
                                        <div key={workspace.id} className="border border-gray-200 rounded-lg p-6 hover:shadow-md transition-shadow w-full">
                                            <div className="flex items-center justify-between mb-4">
                                                ...
                                                <div className="flex items-center space-x-1">
                                                    ...
                                                    <span className="text-sm text-gray-500">{workspace.totalMembers}</span>
                                                </div>
                                            </div>
